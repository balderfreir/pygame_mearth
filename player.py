...
class Player(Entity):
    def __init__(self, pos, groups, obstacle_sprites, create_attack, destroy_attack, create_magic):
        ...
        self.magic_index = 0  # like for weapon
    ...
    def input(self):
        if not self.attacking:
            ...
            if keys[pygame.K_e] and self.can_switch_magic:
                ...
                if self.magic_index < len(list(magic_data.keys())) - 1:  # check if index in range
                    self.magic_index += 1
                else:
                    self.magic_index = 0
                self.magic = list(magic_data.keys())[self.magic_index]  # change index to current one
